...
resource "aws_lb_listener" "http" {
  load_balancer_arn = module.web_alb.arn # Please verify web_alb modules to identify the output of web_alb 
  port              = "80"
  protocol          = "HTTP"
...
module "records" {
  ...
  records = [
    {
      name    = "expense-${var.environment}"  #*.app-dev.aws-devops81s.online
      type    = "A"
      alias   = {
